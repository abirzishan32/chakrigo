...
export default function SocketHandler(_req: NextApiRequest, res: NextApiResponseWithSocket) {
    ...
    io.use(async (socket, next) => {
        try {
            ...
            if (!cookies) {
                console.error('❌ [Socket] No cookies found')
            ...
            if (!sessionCookie) {
                console.error('❌ [Socket] No session cookie found')
                return next(new Error('No session cookie'))
            }

            // Verify the session cookie using Firebase Auth
            ...
            const user = {
                ...userData,
                id: userRecord.id,
            } as any; // Type assertion for socket context
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('notification:send', async (data: {
            ...
            try {
                const { type, message, targetUserId, meta } = data;
                const senderId = socket.data.userId;
                const senderEmail = socket.data.email;

                console.log(`[Notification] ${senderEmail} sending notification:`, { type, message, targetUserId });

                // Import notification action (note: this is not ideal but works for demo)
                // In production, you should handle this differently
                const { createNotification } = await import('@/lib/actions/notification.action');

                const result = await createNotification({
                    type: type as any,
                    message,
                    userId: targetUserId,
                    meta: { ...meta, senderId, senderEmail }
                });

                socket.emit('notification:send:response', result);
            } catch (error) {
                console.error('[Notification] Error sending notification:', error);
                socket.emit('notification:send:response', {
                    success: false,
