...
export default function AssessmentPage({ params }: { params: { id: string } }) {
    ...
    const [assessmentStatus, setAssessmentStatus] =
        useState<AssessmentStatus>("intro");
    ...
    const [environmentRisk, setEnvironmentRisk] = useState<"low" | "high">(
        "low"
    );
    ...
    const handleCheatingDetected = useCallback(
        (reason: string = "Potential academic integrity violation") => {
            ...
            localStorage.setItem(
                `assessment_disqualification_reason_${params.id}`,
                reason
            );
    ...
    useEffect(() => {
        ...
        const handleVisibilityChange = () => {
            if (
                ...
                console.log(
                    "Tab switching detected - triggering disqualification"
                );
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === "s") {
                ...
                toast.error(
                    "Page saving is not allowed during the assessment",
                    { duration: 3000 }
                );
                return;
            }
        ...
        const detectProgrammaticScreenshot = () => {
            ...
            navigator.mediaDevices.getDisplayMedia = function (constraints) {
                ...
                return Promise.reject(
                    new Error(
                        "Screen capture denied by assessment security policy"
                    )
                );
            };
        ...
        const preventPageSaving = () => {
            ...
            document.execCommand = function (
                ...
                if (command === "SaveAs" || command === "Save") {
                    handleCheatingDetected(
                        "Page saving attempt through execCommand"
                    );
                    return false;
                }
            ...
            URL.createObjectURL = function (object: Blob | MediaSource) {
                ...
                if (object instanceof Blob) {
                    ...
                    if (
                        object.type.includes("text/html") ||
                        object.type.includes("application/x-mhtml") ||
                        object.type.includes("text/plain")
                    ) {
                        handleCheatingDetected(
                            "Blob creation attempt for page content"
                        );
                        throw new Error(
                            "Blob creation blocked by assessment security policy"
                        );
                    }
                }
            ...
            const blockBrowserMenus = () => {
                ...
                const handleFocus = () => {
                    ...
                    if (
                        ...
                        console.warn("Potential browser menu access detected", {
                            timeAway,
                        });
    ...
    useEffect(() => {
        ...
        const monitoringInterval = setInterval(async () => {
            ...
            if (
                newResult &&
                (newResult.isRemoteAccess || newResult.isVirtualMachine)
            ) {
                ...
                if (environmentRisk !== "high") {
                    ...
                    handleCheatingDetected(
                        "Environment security violation detected during assessment"
                    );
                }
            }
            ...
            try {
                ...
                if (response && response.success && "data" in response) {
                    ...
                    if (
                        ...
                        const questionObjects = response.data.questions.filter(
                            (q) => q && typeof q === "object" && "id" in q
                        );
                        console.log(
                            "Filtered question objects:",
                            questionObjects
                        );

                        if (questionObjects.length > 0) {
                            setQuestions(
                                questionObjects as AssessmentQuestion[]
                            );
                        } else {
                            // If no question objects found, try to load them from assessmentQuestions collection
                            const questionIds = response.data.questions.filter(
                                (q) => typeof q === "string"
                            );
                            ...
                            try {
                                ...
                                if (validQuestions.length > 0) {
                                    setQuestions(
                                        validQuestions as AssessmentQuestion[]
                                    );
                                } else {
                                    setError(
                                ...
                                console.error(
                                    "Error fetching individual questions:",
                                    fetchError
                                );
    ...
    useEffect(() => {
        if (
            ...
            const hasSecurityConcerns =
                detectionResult.isRemoteAccess ||
                detectionResult.isVirtualMachine;
    ...
    useEffect(() => {
        ...
        if (startTimeRef.current === null) {
            ...
            localStorage.setItem(
                `assessment_end_time_${params.id}`,
        ...
        timerRef.current = setInterval(() => {
            const endTime = parseInt(
                localStorage.getItem(`assessment_end_time_${params.id}`) || "0"
            );
    ...
    const prepareToStartAssessment = () => {
        ...
        if (questions.length === 0) {
            toast.error(
                "This assessment has no questions yet. Please try another assessment or check back later."
            );
    ...
    const handleEnvironmentCancel = () => {
        ...
        toast.error(
            "Assessment cancelled due to environment security concerns"
        );
    };
    ...
    const goToPreviousQuestion = () => {
        if (currentQuestionIndex > 0) {
            ...
            const currentTime = Date.now();
            const startTime =
                questionStartTimes[currentQuestion.id] || currentTime;
    ...
    const calculateAndShowResults = async () => {
            ...
            if (userAnswer) {
                if (question.type === "multiple-choice") {
                    ...
                    if (question.answerType === "single") {
                        ...
                        const hasNoIncorrect = userAnswer.selectedOptions.every(
        ...
        const totalTimeSpent = Object.values(questionTimings).reduce(
        ...
        if (session?.user) {
            try {
                ...
                const recommendationsResult =
                    await generateStudyRecommendationsAgent({
    ...
    const renderStudyRecommendations = () => {
        ...
        if (
            ...
            studyRecommendations.topics &&
            studyRecommendations.topics.length > 0
        ) {
            return (
                <div className="mt-8">
                    ...
                    <div className="space-y-6">
                        {studyRecommendations.topics.map(
        ...
        const incorrectQuestions =
            result?.answersByQuestion.filter((q) => !q.correct) || [];
                ...
                <div className="mt-8">
                    ...
                    <div className="space-y-6">
                        <div className="p-6 bg-gray-800 rounded-lg">
                            ...
                            <p className="text-gray-300 mb-4">
                                Pay special attention to these topics that
                                appeared in questions you found challenging.
                            </p>
                                ...
                                {keywords.map((keyword, idx) => (
                                    <div key={idx} className="flex items-start">
                                        ...
                                        <span className="text-gray-300">
                                            Focus on basics of {keyword} and
    ...
    const renderTimeAnalytics = () => {
        ...
        const sortedQuestions = [...result.answersByQuestion]
            .sort(
                (a, b) =>
                    (questionTimings[b.questionId] || 0) -
                    (questionTimings[a.questionId] || 0)
            )
            .slice(0, 3); // Top 3 time-consuming questions

        // Find questions that were answered incorrectly
        const incorrectQuestions = result.answersByQuestion.filter(
            (q) => !q.correct
        );
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className="bg-gray-800 rounded-lg p-4">
                            ...
                            <div className="text-xl font-bold text-white">
                                {Math.floor(
                                    totalTimeSpent /
                                        result.answersByQuestion.length /
                                        60
                                )}
                                m{" "}
                                {Math.round(
                        ...
                        <div>
                            ...
                            <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-lg">
                                <p className="text-gray-300 text-sm mb-2">
                                    You answered {incorrectQuestions.length}{" "}
                                    question
                                    {incorrectQuestions.length > 1 ? "s" : ""}{" "}
                                    incorrectly. Focus on studying these topics
                                ...
                                {incorrectQuestions.length <= 3 ? (
                                    ...
                                    <div className="mt-2 space-y-2">
                                        ...
                                        <div className="mt-2 text-sm text-blue-400">
                                            Review the study recommendations
                                            below for detailed topics to improve
                                            your understanding.
                                        </div>
                                    </div>
                                )}
        ...
        <EnvironmentDetectionProvider
            ...
            onEnvironmentRisk={(riskLevel, details) => {
                ...
                if (
                    ...
                    handleCheatingDetected(
                        "High-risk environment detected during assessment"
                    );
                }
            }}
        >
                        ...
                        <div>
                            ...
                            <div className="bg-gray-900 border border-gray-800 rounded-xl shadow-xl overflow-hidden">
                                <div className="p-6 border-b border-gray-800">
                                    ...
                                    <p className="text-gray-300 text-sm">
                                        Verifying your environment meets
                                    ...
                                    {environmentLoading ? (
                                        ...
                                        <div className="text-center py-8">
                                            ...
                                            <p className="text-gray-400 mb-6">
                                                ...
                                                environment. Please refresh and
                                                try again.
                                            </p>
                                            <button
                                                onClick={() => {
                                                ...
                                                {currentQuestion.options?.map(
                                                    (option) => {
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                <div
                                                                    className={`w-6 h-6 flex items-center justify-center rounded-full mr-3 transition-colors ${
                                                                        isSelected
